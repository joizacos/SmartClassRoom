 ...
 */
char I2CReadReg(BYTE devAddr, BYTE regAddr, unsigned int rwDelay)
{
	unsigned char v;
	devAddr = devAddr << 1;
 	I2CStart();						//Start sequence
	I2CWrite(devAddr & 0xFE);		//Initiate write sequence
	I2CWrite(regAddr);				//Send register (r) to read
	I2CRestart();					//Restart
 ...
 * \return the report for the operation.
 */
BOOL I2CReadMulti(BYTE devAddr, BYTE regAddr, BYTE dest[], unsigned int regToRead, unsigned int rwDelay)
{
	unsigned char rep;
	BOOL report;
	...
	for (rep=0; rep<(regToRead-1); rep++)
	{
  	...
  	dest[rep] = I2CRead(1);					//Receive lsb
 ...
 * \return None.
 */
void I2CWriteReg(BYTE devAddr, BYTE regAddr, BYTE val)
{
	devAddr = devAddr << 1;
 	I2CStart(); 					//Start sequence
	I2CWrite(devAddr & 0xFE);		//Initiate write sequence
	I2CWrite(regAddr);				//Send register (regAddr) to write
 ...
 * \return None.
 */
void I2CWriteMulti(BYTE devAddr, BYTE regAddr, BYTE* src, unsigned int regToWrite)
{
	unsigned char rep;
