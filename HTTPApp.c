...
// Callback function for the static variable ~temp~.
extern double temp;

void HTTPPrint_temp()
{
	UARTWrite(1, "Processing request...");
	char auxiliar [50];
	sprintf(auxiliar,"%2.1f ºC",temp);
	// Print the output
	TCPPutString(sktHTTP, (BYTE*)auxiliar);
	return;
}
